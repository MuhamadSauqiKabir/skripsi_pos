...
const sendToWhatsApp = () => {
    ...
    const text = `Halo Nineties Coffee, saya ingin reservasi atau bertanya:\n\n*Nama:* ${form.value.name}\n*WA:* ${form.value.phone}\n*Email:* ${form.value.email || '-'}\n\n*Pesan:*\n${form.value.message}`;
    ...
    <section class="mx-auto max-w-7xl px-6 py-20">
        <div class="grid gap-10 lg:grid-cols-[0.9fr_minmax(0,1fr)]">
            <div class="rounded-3xl bg-brand-espresso p-10 text-white shadow-soft">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-gold">Kontak</p>
                ...
                <div class="mt-10 grid gap-6 text-sm leading-7 text-white/75">
                    ...
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-gold">Jam Operasional</h3>
            ...
            <div class="rounded-3xl border border-brand-gold/10 bg-white p-10 shadow-soft">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-gold">Reservasi dan Pertanyaan</p>
                <h2 class="mt-4 font-serif text-4xl font-bold text-brand-espresso">Mari bicara soal meja, acara, atau kolaborasi.</h2>
                <form @submit.prevent="sendToWhatsApp" class="mt-8 grid gap-5">
                    ...
                    <textarea v-model="form.message" required rows="5" placeholder="Detail reservasi atau pesan Anda..." class="rounded-2xl border-brand-gold/20 bg-brand-mist focus:border-brand-gold focus:ring-1 focus:ring-brand-gold"></textarea>
